...
) : host_(host), port_(port), username_(username), password_(password), type_(type), orderBooks_ref_(orderBooks_ref),
    running_(false), is_logined_(false), sock_(INVALID_SOCKET) {}
...
bool L2TcpSubscriber::connect() {
    ...
    int sent = send(sock_, loginCmd.c_str(), static_cast<int>(loginCmd.size()), 0);
    ...
    if (n == SOCKET_ERROR) {
        ...
        LOG_ERROR(module_name, "TCP 接收登录响应出现错误 <{}:{}>，错误码: {}", host_, port_, err);
        closesocket(sock_);
        sock_ = INVALID_SOCKET;
        return false;
    }
    ...
    if (isContainStrFlag(buffer, n, "Login successful")) {
        ...
        LOG_ERROR(module_name, "登录失败，服务器返回: {} <{}:{}>", response, host_, port_);
        // 登录失败，关闭连接，等待下一次尝试
        closesocket(sock_);
        sock_ = INVALID_SOCKET;
...
std::string L2TcpSubscriber::recvData() {
    ...
    if (n == SOCKET_ERROR) {
        int err = WSAGetLastError();
        LOG_ERROR(module_name, "TCP 接收数据出现错误 <{}:{}>，错误码: {}", host_, port_, err);
        return "";
    }
    ...
    if (isContainStrFlag(buffer, n, "Order") || isContainStrFlag(buffer, n, "Tran")) {
        return "1";
    }
...
void L2TcpSubscriber::receiveLoop() {
    ...
    while (running_) { // 检查全局运行标志
        ...
        if (data.empty()) {
            LOG_WARN(module_name, "接收线程检测到连接断开，准备重连 <{}:{}>", host_, port_);
            is_logined_ = false; // 设置状态为未登录
