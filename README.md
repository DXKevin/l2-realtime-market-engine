...
*   **时间优先 (std::list)**: 同一价位下的订单采用双向链表存储，严格遵循交易所的 **FIFO** 成交原则。
*   **快速索引 (std::unordered_map)**: 核心亮点——存储 `order_id` 到 `std::list::iterator` 的映射。凭借 `std::list` 迭代器永不失效的特性，实现 **$O(1)$** 复杂度的订单定位、成交处理及撤单删除。

### 2. 并发与异步模型
*   **MPSC 消息队列**: 采用 `moodycamel::BlockingConcurrentQueue`，网络接收线程（多生产者）并发压入事件，计算线程（单消费者）顺序处理，彻底消除锁竞争。
*   **乱序容错机制**: 内置 `pending_trade_events` 缓冲区。针对交易所可能存在的成交早于委托到达的微小时间差（或历史数据空隙），实现自动挂起与重触发处理。
